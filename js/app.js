...
const yestardayTime = new Date(Date.now() - 86400000)
  .toISOString()
  .slice(0, 10);
let ukraineData = [];
let worldData = [];
...
wrapperTabsEl.addEventListener("click", (e) => {
  ...
  Array.from(wrapperTabsEl.children).forEach((btn) =>
    btn.classList.remove("active")
  );
  if (tab) {
    tab.classList.add("active");
    const region = tab.dataset.region;
    Array.from(wrapperContainerTabsEl.children).forEach((tabPage) =>
      tabPage.classList.remove("active_tab")
    );
...
function yesterdayFetchFromDataCenter(url) {
  ...
  .then((res) => res.json())
  .then((data) => {
    // yestardayConfirmed = data.ukraine.map((res) => res.confirmed);
    yestardayUkraineData = data.ukraine;


    console.log(createSmartKeyForDifferenceCount(ukraineData, yestardayUkraineData, keys))

// console.log(createDifferenceFieldsets(yestardayConfirmed));
...
function createSmartKeyForDifferenceCount(dataArrayFirst, dataArraySecond, smartKeys) {
  return smartKeys.map(smartKey => createYestardayData(dataArrayFirst, dataArraySecond, smartKey))
}
...
function createDifferenceFieldsets(fieldsets) {
  return fieldsets.map(field => createDifferenceField(field)).join('')
}

function createDifferenceField(field) {
  return `${field > 0 ? `<p>&#8593;${field}</p>` : `<p>&#8595;${field}</p>`}`
}
